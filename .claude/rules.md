...
4. **User input is untrusted at the boundary.** Validate `url` (scheme must be
   http/https), validate `json_schema` is a real JSON Schema object, cap
   `prompt` length. Do this in `models.py` with Pydantic, not with ad-hoc ifs
   scattered around.
5. **Never interpolate user input into SQL.** Parameterized queries only.

## B. Module boundaries

6. **Respect the import table in `architecture.md` §2.** `recon.py` never calls
   the LLM. `generate.py` never touches Playwright. `executor.py` never talks
   to the LLM. `retry_loop.py` is the only orchestrator.
...
8. **Schema validation lives in the harness.** Generated scripts return raw
   dicts. `executor.py` validates. Never duplicate schema logic into the LLM
   prompt.
...
13. **Recon output stays compact.** If raw HTML reaches `generate.py`, that is a
...
15. **Reuse before writing.** Check `models.py` and existing helpers first. The
...
27. **Follow `design.md`.** Tokens at `:root`, no hardcoded colors, mono for
